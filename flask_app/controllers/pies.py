...
def viewPie(id):
    if 'user_id' in session:
        ...
        # votedPies = User.get_user_voted_pies(data)
        pies = Pie.get_all()
        loggedUserVotedPies = User.get_user_voted_pies(data)
    return render_template('showOne.html',pie=pie, pies = pies,user=user, loggedUser=loggedUser, votedpies = loggedUserVotedPies)
    return redirect('/')

@app.route('/edit/pie/<int:id>', methods = ['POST'])
def updatePie(id):
    if 'user_id' in session:
        data1 = {
            'user_id': session['user_id'],
...
def votePie(id):
    if 'user_id' in session:
        ...
        votedPost = User.get_user_voted_pies(data)
        if id not in votedPost:
            Pie.addVote(data)
            return redirect(request.referrer)
